...
class FeatureController extends Controller {
    ...
    async validateFeatureToggleName(name) {
        let hasError = false;
        if (!name.includes('.')) {
            hasError = true;
    ...
    async validateUniqueName(name) {
        let msg;
        try {
            const hasError = await this.validateFeatureToggleName(name);

            if (hasError) {
                msg =
                    'Invalid name format. Please enter feature name as: <cas_name>.<env>.<feature_toggle_name>';
            } else {
                const definition = await this.featureToggleStore.hasFeature(
                    name,
                );
